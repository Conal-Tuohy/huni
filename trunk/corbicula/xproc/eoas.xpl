...
<!--
	<p:for-each name="deletion">
		...
		<p:store>
			<p:with-option name="href" select="concat('/data/xml/eoas/deletions/', /*/@xml:base)"/>
		</p:store>
	</p:for-each>
	<p:for-each name="update">
		...
		<p:store>
			<p:with-option name="href" select="concat('/data/xml/eoas/updates/', /*/@xml:base)"/>
		</p:store>
	</p:for-each>	
	<p:for-each name="error">
		...
		<p:store>
			<p:with-option name="href" select="concat('/data/xml/eoas/errors/error-', position(), '.xml')"/>
		</p:store>
	</p:for-each>		
-->	
	<!--
	
	<p:for-each name="record-to-transform">
		<p:iteration-source>
			<p:pipe step="harvest" port="updates"/>
		</p:iteration-source>
		<drone:xml-to-rdf xslt="awr-eac-cpf-to-rdf.xsl"/>
		<p:store>
			<p:with-option name="href" select="concat('/data/xml/awr/graphs/', fn:encode-for-uri(/*/@xml:base), '.xml')"/>
		</p:store>
	</p:for-each>
	-->
	<p:for-each name="transform-record-to-rdf">
		...
		<drone:xml-to-rdf name="rdf" xslt="esrc-eac-to-rdf.xsl" resource-base-uri="http://corbicula.huni.net.au/data/eoas/"/>
